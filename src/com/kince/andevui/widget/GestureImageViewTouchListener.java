...
import com.kince.andevui.animation.FlingAnimation;
import com.kince.andevui.animation.FlingAnimationListener;
import com.kince.andevui.animation.FlingListener;
import com.kince.andevui.animation.MoveAnimation;
import com.kince.andevui.animation.MoveAnimationListener;
import com.kince.andevui.animation.ZoomAnimation;
import com.kince.andevui.animation.ZoomAnimationListener;
import com.kince.andevui.util.MathUtils;
import com.kince.andevui.util.VectorF;
...
public class GestureImageViewTouchListener implements OnTouchListener {
		...
		if (!inZoom) {
			...
			if (!tapDetector.onTouchEvent(event)) {
				...
				if (event.getAction() == MotionEvent.ACTION_UP) {
					...
					if (event.getPointerCount() > 1) {
						multiTouch = true;
						MyViewpager.multiTouchflag =true;
						...
						MyViewpager.multiTouchflag =false;
						if (!touched) {
							touched = true;
							last.x = event.getX();
							last.y = event.getY();
	...
	protected void calculateBoundaries() {
		...
		// 在这里加判断
		MyViewpager.notflag = canDragX || canDragY;
