...
package com.kince.andevui.widget.scrollbarpanellib;
...
public class Clock extends View {
	...
	public Clock(Context context, AttributeSet attrs, int defStyle) {
		 ...
		 * mHourHand =
		 * resource.getDrawable(R.drawable.ic_timer_clock_hour_hand);
		 * mMinuteHand =
		 * resource.getDrawable(R.drawable.ic_timer_clock_minute_hand);
		 * mSecondHand =
		 * resource.getDrawable(R.drawable.ic_timer_clock_minute_hand);
		 */
		final int scrollBarPanelLayoutId = a.getResourceId(
				R.styleable.Clock_scrollBarPanel, -1);

		System.out.println("***** scrollBarPanelLayoutId ***********"
				+ scrollBarPanelLayoutId);
		/**
		 * dialer background
		 */
		...
		if (hourRes != -1) {
			...
			mHourHand = resource
					.getDrawable(R.drawable.ic_timer_clock_hour_hand);
		}
		...
		if (minuteRes != -1) {
			...
			mMinuteHand = resource
					.getDrawable(R.drawable.ic_timer_clock_minute_hand);
		}
	 ...
	 */
	public void setImages(Drawable dial, Drawable hourHand,
			Drawable minuteHand, Drawable secondHand,
			boolean isSeconeHandVisible) {
		mDial = dial;
		mHourHand = hourHand;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (availableWidth < mDialWidth || availableHeight < mDialHeight) {
			scaled = true;
			float scale = Math.min((float) availableWidth / (float) mDialWidth,
					(float) availableHeight / (float) mDialHeight);
			canvas.save();
		...
		if (changed) {
			mDial.setBounds(x - (mDialWidth / 2), y - (mDialHeight / 2), x
					+ (mDialWidth / 2), y + (mDialHeight / 2));
		}
		...
		if (changed) {
			...
			hourHand.setBounds(x - (hourHandWidth / 2), y
					- (hourHandHeight / 2), x + (hourHandWidth / 2), y
					+ (hourHandHeight / 2));
		}
		...
		if (changed) {
			...
			minuteHand.setBounds(x - (minuteHandWidth / 2), y
					- (minuteHandHeight / 2), x + (minuteHandWidth / 2), y
					+ (minuteHandHeight / 2));
		}
		...
		if (isSecondHandVisible) {
			...
			mSecondHand.setBounds(x - (secondHandWidth / 2), y
					- (secondHandHeight / 2), x + (secondHandWidth / 2), y
					+ (secondHandHeight / 2));
		}
